...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, d_k, dropout=0.01):
    ...
    def forward(self, q, k, v, attn_mask=None):
        # q: [b_size x n_heads x len_q x d_k]   len: 字符个数（512， 512）， d_k: 字符特征维度（512， 1024）
        # k: [b_size x n_heads x len_k x d_k]
        # v: [b_size x n_heads x len_v x d_v] note: (len_k == len_v) (d_k == d_q)

        # attn: [b_size x n_heads x len_q x len_k]
        scores = torch.matmul(q, k.transpose(-1, -2)) / self.scale_factor  # scaled scores
        if attn_mask is not None:
            assert attn_mask.size() == scores.size()
            scores.masked_fill_(attn_mask, -1e9)
        attn = self.dropout(self.softmax(scores))  # Attention
...
class PosEncoding(nn.Module):    #useless
