...
from utils.modules import ScaledDotProductAttention
from utils.modules import LayerNormalization


class _MultiHeadAttention(nn.Module):
    def __init__(self, d_k, d_v, d_model, n_heads, dropout=0.01):
        super(_MultiHeadAttention, self).__init__()
        self.d_k = d_k
        self.d_v = d_v
        self.d_model = d_model  # 1024 (512*2)
        self.n_heads = n_heads

        self.w_q = Linear(d_model, d_k * n_heads, True)  # d_q == d_k
        self.w_k = Linear(d_model, d_k * n_heads, True)
        self.w_v = Linear(d_model, d_v * n_heads, True)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        context, attn = self.multihead_attn(q, k, v)
